...
let rec type_stmt env stmt =
   ...
   | Ast.Swhile (e, bloc) ->
         let te, always_return = type_expr env e in
         let tbloc, t, _ = type_bloc env bloc in
         let () = TypCheck.typ te.te_localisation Tbool te.te_typ in
         let () = TypCheck.typ te.te_localisation Tunit t in
         (
            { te_expr   = Ewhile (te, tbloc) ;
              te_typ          = t ;
              te_lval         = false ;
              te_mut          = false ;
   ...
   | Ast.Sif (e, b, else_comp_opt) ->
         let te, _ = type_expr env e in
         let then_tbloc, then_typ, then_always_return = type_bloc env b in
         let else_tbloc, else_typ, else_always_return =
            match else_comp_opt with
            | None                   -> ([], Tunit, false)
            | Some (Ast.Cbloc b)     -> type_bloc env b
            | Some (Ast.Cif ifcomp)  -> type_bloc env ([Ast.Sif ifcomp], None)
         in
         let tifcomp = 
            { cond = te ;
              then_bloc = then_tbloc ;
              else_bloc = else_tbloc ;
            }
         in
         let () = TypCheck.typ te.te_localisation Tbool te.te_typ in
...
and type_bloc env bloc =
   ...
   | h :: t, opt  ->
         ...
         let tbloc, t, always_return2 = type_bloc new_env (t, opt) in
         (te :: tbloc, t, always_return1 || always_return2)
   
and type_expr env { Ast.expression; Ast.localisation } =
   match expression with
   | Ast.Ebloc bloc           -> 
         let tbloc, t, always_return = type_bloc env bloc in
         (
         { te_expr = Ebloc tbloc ;
           te_typ = t ;
           te_lval = false ;
...
let type_fun env (ast_decl_fun : Ast.decl_fun) =
   ...
   let tbloc, actual_rty, always_return = 
      type_bloc fun_env ast_decl_fun.Ast.body 
   in
   let () = 
   ...
   let fun_def = { decl = fun_decl ; args = il ; body = tbloc } in
   new_env, fun_def
